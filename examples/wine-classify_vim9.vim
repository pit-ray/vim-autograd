...
def  Linear(x: any, W: any, b: any = {}): any
  var t = ag.Matmul(x, W)
  return empty(b) ? t : ag.Add(t, b)
enddef
...
def Softmax(x: any): any
  var y = ag.Exp(x.s(ag.Max(x)))
  var s = ag.Sum(y, 1, 1)
  return ag.Div(y, s)
enddef

def CrossEntropyLoss(y: any, t:any): any
  var loss = ag.Mul(a:t, ag.Log(a:y))
  var batch_size = loss.shape[0]
  return ag.Div(ag.Sum(loss), batch_size).n()
enddef

function! MLP_forward(x) abort
  let y = Linear(a:x, self.params[0], self.params[1])
  for l:i in range(2, len(self.params) - 1, 2)
    let y = Relu(y)
    let y = Linear(y, self.params[l:i], self.params[l:i + 1])
  endfor
  let y = Softmax(y)
  return y
endfunction

def MLP(in_size: number, ...layer_sizes: list<number>): any
  var mlp = {
    'params': [],
    'forward': function('MLP_forward')
  }

  var std = sqrt(2.0 / in_size)
  var W = ag.Normal(0, std, [in_size, layer_sizes[0]])
  var b = ag.Zeros([layer_sizes[0]])
  W.name = 'W0'
  b.name = 'b0'
  mlp.params += [W, b]

  for i in range(len(layer_sizes) - 1)
    var std = sqrt(2.0 / layer_sizes[i])

    var W = ag.Normal(0, std, [layer_sizes[i], layer_sizes[i + 1]])
    W.name = 'W' . string(i + 1)

    var b = ag.Zeros([layer_sizes[i + 1]])
    b.name = 'b' . string(i + 1)
